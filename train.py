...
from datagenerator import Cylinder_DataGenerator
from datagenerator import CylinderSteady_DataGenerator, CylinderUnSteady_DataGenerator, Hasanuzzaman_DataGenerator
from datagenerator import wufan_DataGenerator
from datagenerator import LDCSteady_DataGenerator
...
def train_2d2c_liuyi_fsi():
    """
    PINN for 2d2c of oscillatory cylinder
    """
...
def train_2d2c_yangyb():
    """
    PINN for 2d2c of yangyb
    """
    data_pathname = './data/yangyb'
    data_filename = '2d2c_yangyb_jet_v2.mat'
    # 相同的数据格式
    data, norm_paras = wufan_DataGenerator(data_pathname, data_filename)  
...
def train_2d2c_zangzy():
    """
    PINN for 2d2c of zangzy
    """
...
def train_2d2c_cylinder_wufan():
    """
    PINN for 2d2c of oscillatory cylinder
    """
    data_pathname = './data/wufan'
    nlevels = [16,0,8,4,2,1]
    nlevels = [8]
    reslus = [5,10,20,40,80]
    reslus = [40]
    rnum = [0]
    for nlev in nlevels:
        for nres in reslus:
            data_filename = 'wufan_movingcylinder2D_reslu{nres}_noise{nlev}_pinn.mat'.format(nres=nres,nlev=nlev)
            # 相同的数据格式
            data, norm_paras = wufan_DataGenerator(data_pathname, data_filename)  
            ...
            """
            for runidx in rnum: 
                savename = 'wufan_movingcylinder2D_13_128_reslu{nres}_noise{nlev}_pinn_run{runidx}'.format(nres=nres,nlev=nlev,runidx=runidx)
...
#     """
#     data_pathname = './data/bendpipe'
#     data_filename = 'NewNew3d3c_InRe392p5wpi_4_dt0.008_Bendpipe.mat'
...
#         parameters
#         """
#         savename = 'NewNew3d3c_InRe392p5wpi_4_13_128_run{runidx}'.format(runidx=runidx)
...
if __name__ == "__main__":
    ...
    # train_2d2c_mitralvalve_fsi()
    
    # train_3d3c_mitralvalve_fsi()
    
    # train_2d2c_wufan_fsi()
    
    # train_2d2c_yangyb()
    
    # train_2d2c_liuyi_fsi()
    
    # train_2d2c_zangzy()
    
    # train_2d2c_cylinder_wufan()

    # train_2d2c_bendpipe_zxc()
