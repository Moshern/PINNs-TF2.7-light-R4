...
def Cylinder_DataGenerator(data_pathname):
    """
    Suboff 2D
    """
    # Please use HDF reader for matlab v7.3 files
    # data = sio.loadmat(os.path.join(data_pathname, data_filename))
    data_filename = 'cylinder_points_2d.mat'
    tmp = h5py.File(os.path.join(data_pathname, data_filename),'r')
    eqns = np.transpose(tmp['data_eqns'])
    
    bc_uv = np.transpose(tmp['bc_uv'])
    #bc_v = np.transpose(tmp['bc_v'])
    bc_init = np.transpose(tmp['bc_init'])
    
    bc_ux = np.transpose(tmp['bc_ux'])
    #bc_uy = np.transpose(tmp['bc_uy'])
    bc_vx = np.transpose(tmp['bc_vx'])
    bc_px = np.transpose(tmp['bc_px'])
    bc_py = np.transpose(tmp['bc_py'])
    bc_p = np.transpose(tmp['bc_p'])
    ...
    return eqns, bc_init, bc_uv, bc_ux, bc_vx, bc_p, bc_px, bc_py, norm_paras





def CylinderSteady_DataGenerator(data_pathname):
    """
    steady cylinder
    """
    # Please use HDF reader for matlab v7.3 files
    # data = sio.loadmat(os.path.join(data_pathname, data_filename))
    data_filename = 'cylinder_steady_cfd_points_2d.mat'
    tmp = h5py.File(os.path.join(data_pathname, data_filename),'r')
    ...
    data_dict['bc_init'] = np.transpose(tmp['bc_init'])
    data_dict['bc_p'] = np.transpose(tmp['bc_p'])
    data_dict['bc_px'] = np.transpose(tmp['bc_px'])
    data_dict['bc_py'] = np.transpose(tmp['bc_py'])
        
    # 数据归一化参数
    # [min(t),min(x),min(y),mean(u),mean(v),mean(p)]
    # [max(t),max(x),max(y),std(u),std(v),std(p)]
    norm_paras = np.zeros([2,5])    
    eqns =  np.transpose(tmp['data_eqns'])
    norm_paras[0,0:2] = eqns[:,0:2].min(0)
    norm_paras[1,0:2] = eqns[:,0:2].max(0)
    norm_paras[0,2:5] = 0
    norm_paras[1,2:5] = 1
...
def CylinderUnSteady_DataGenerator(data_pathname):
    """
    unsteady cylinder
    """
    # Please use HDF reader for matlab v7.3 files
    # data = sio.loadmat(os.path.join(data_pathname, data_filename))
    data_filename = 'wufan_fixcylinder2D_center_30_uniform.mat'
    tmp = h5py.File(os.path.join(data_pathname, data_filename),'r')
    ...
    data_dict['data_eqns'] = np.transpose(tmp['data_eqns'])
    data_dict['bc_uv'] = np.transpose(tmp['bc_supervised'])
...
def wanglei_DataGenerator(data_pathname):
    """
    steady cylinder
    """
    # Please use HDF reader for matlab v7.3 files
    # data = sio.loadmat(os.path.join(data_pathname, data_filename))
    data_filename = '3d3c_wanglei_1_120.mat'
    tmp = h5py.File(os.path.join(data_pathname, data_filename),'r')
...
def LDCSteady_DataGenerator(data_pathname):
    """
    LDC 2D
    """
    # Please use HDF reader for matlab v7.3 files
    # data = sio.loadmat(os.path.join(data_pathname, data_filename))
    data_filename = 'LDC_points_2d.mat'
    tmp = h5py.File(os.path.join(data_pathname, data_filename),'r')
    keys = ['data_eqns', 'data_supervised',
            'bc_uv', 'bc_init',
            'bc_u',  'bc_v', 'bc_p',
    ...
    eqns = np.transpose(tmp['data_eqns'])
    data_dict['bc_uv'] = np.transpose(tmp['bc_uv'])
    data_dict['maxx'] = np.transpose(tmp['maxx'])
    data_dict['minx'] = np.transpose(tmp['minx'])
    data_dict['maxy'] = np.transpose(tmp['maxy'])
    data_dict['miny'] = np.transpose(tmp['miny'])
    ...
    norm_paras = np.zeros([2,5])    
    norm_paras[0,0:2] = eqns[:,0:2].min(0)
    norm_paras[0,0] = data_dict['minx']
    norm_paras[0,1] = data_dict['miny']
    norm_paras[1,0:2] = eqns[:,0:2].max(0)
    norm_paras[1,0] = data_dict['maxx']
    norm_paras[1,1] = data_dict['maxy']
    norm_paras[0,2:5] = 0
    norm_paras[1,2:5] = 1
...
def wufan_DataGenerator(data_pathname, data_filename):
    ...
    # Please use HDF reader for matlab v7.3 files
    # data = sio.loadmat(os.path.join(data_pathname, data_filename))
    # data_filename = 'wufan_movingcylinder2D_reslu40_noise4_wallrefined.mat'
    tmp = h5py.File(os.path.join(data_pathname, data_filename),'r')
    ...
    # data_dict['bc_px'] = np.transpose(tmp['bc_px'])
    # data_dict['bc_ux'] = np.transpose(tmp['bc_ux'])
    # data_dict['bc_vx'] = np.transpose(tmp['bc_vx'])
...
if __name__ == "__main__":
    ...
    # str = './data/cylinder/2D'
    # eqns, bc_init, bc_uv, bc_ux, bc_vx, bc_p, bc_px, bc_py, norm_paras = Cylinder_DataGenerator(str)  
    # fig = plt.figure()
    # ax = fig.add_subplot(111,projection='3d')
    # ax.scatter(bc_py[0:-1:1,0],bc_py[0:-1:1,1],bc_py[0:-1:1,2])
